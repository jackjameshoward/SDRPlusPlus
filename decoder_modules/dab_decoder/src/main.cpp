...
public:
    M17DecoderModule(std::string name)  {
        ...
        // Initialize VFO
        vfo = sigpath::vfoManager.createVFO(name, WaterfallVFO::REF_CENTER, 0, VFO_BANDWIDTH, INPUT_SAMPLE_RATE, VFO_BANDWIDTH, VFO_BANDWIDTH, true);
    ...
    void enable() {
        ...
        vfo = sigpath::vfoManager.createVFO(name, WaterfallVFO::REF_CENTER, std::clamp<double>(0, -bw / 2.0, bw / 2.0), VFO_BANDWIDTH, INPUT_SAMPLE_RATE, VFO_BANDWIDTH, VFO_BANDWIDTH, true);
        vfo->setSnapInterval(250);
